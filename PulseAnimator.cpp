...
PulseAnimator::PulseAnimator(CRGB forwardColor, CRGB backwardColor, CRGB leftColor, CRGB rightColor, uint32_t period) {
	_period = period;
...
bool PulseAnimator::Execute(uint32_t micros, bool modeChanged, CLEDController ** controllers, PositionFlags flags[], uint32_t numControllers) {
	...
	for (uint32_t i = 0; i < numControllers; i++) {
		...
		int ix = min(idx, numLeds - 1);

		for (uint8_t j = 0; j < numLeds; j++) {
			int x = reverse ? numLeds -1 - j : j;
			buffer[x] = (ix == j) ? CRGB::White : color;
		}
	}
...
CRGB PulseAnimator::GetColor(PositionFlags flags) {
